...
# replace2
aug_g, aug_feat = augmentor.generate('replace2', ratio=0.4)
aug_g = aug_g.to(device)
aug_feat = aug_feat.to(device)
aug_h = conv(aug_g, aug_feat)
...
consis = aug_acc
divers = torch.norm((pred[val] - aug_pred[val]), p='fro').item()
print('replace2', aug_acc, divers)

# augmentation
for strategy in ['dropedge', 'dropnode', 'dropout']:
    # generate augmentation
    aug_g, aug_feat = augmentor.generate(strategy, ratio=0.4)
    aug_g = aug_g.to(device)
    aug_feat = aug_feat.to(device)
    aug_h = conv(aug_g, aug_feat)
