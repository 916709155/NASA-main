...
parser.add_argument('--dataset', default='pubmed') #数据集
parser.add_argument('--lr', type=float, default=0.01, help='Initial learning rate.') #学习率
parser.add_argument('--weight_decay', type=float, default=1e-3, help='Weight decay') #权重衰减
parser.add_argument('--epochs', type=int, default=1, help='Number of epochs to train.') #epoch
parser.add_argument('--hidden', type=int, default=32, help='Number of hidden units.') #隐藏层单元
parser.add_argument('--dropout', type=float, default=0.8, help='Dropout rate') #脱出率
parser.add_argument('--alpha', type=float, default=0.2, help='loss hyperparameter') #正则化参数
...
for epoch in range(args.epochs):
    ...
    aug_g, aug_feat = augmentor.generate(strategy='NRGE', num_neighbor=2, ratio=args.prob)
